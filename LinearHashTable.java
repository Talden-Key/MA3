...
class LinearHashTable<K, V> extends HashTableBase<K, V>
{
    ...
    public void addElement(K key, V value)
    {
        ...
        int i = hash;
        // do the linear function until find the the table is full or until the item is inserted.
        while ( i != hash )  {
            // if we find the key with an empty slot add the value to the key.
            ...
            // we find the key are the same and is occupied, then return to the function to find the next key slot.
            if (slot.equals(key)) {
                slot.setValue(value);
                _number_of_elements++;
                return;
            }
            i = (i +1) % super.TableSize();
        }
    ...
    public void removeElement(K key)
    {
        ...
        // if there is no key return false
            if (slot.isEmpty()){
                return;
            }// else find the key and delete the item
            else { slot.setValue(null); }
            ...
            //  rehash all  of the keys to nul
            for ( int i = 0; i > super.TableSize(); i = (i+1) % super.TableSize()) {
                new_hash.HashItem(null, null);
            }

            // add the element from the current hashtable to the new hashtable.
            for (int i = 0; i < super.TableSize(); i ++){
                HashItem<K, V> current = _items.elementAt(i);
                new_hash.addElement(current.getKey(), current.getValue());
            }
        //
